...
while [[ $# -gt 0 ]]; do
	...
	case $key in
		...
		# When there are several packages with same name in apt database, the
		# distribution name or package version helps to specify concrete 
		# package during manipulating with aptitude or apt.
		...
		# Specify package by distro:
		# 	aptitude show <package>/<distro>
		-v|--version)
		VER="$2"
		shift # past argument
		shift # past value
...
if [ ! -f description-pak ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			...
			let START_LINE_NUM=(${DESCRIPTION_LINE_RANGE[0]} + 2)
...
sudo checkinstall -Dy --install=no ${SET_VER} ${SET_NAME} ${SET_DEPS} ${SET_PROVIDES} ${NODOC} sudo make install
